...
class ConvBNN(torch.nn.Module):
    ...
    def __init__(self, channels=[1,64,64], kernels=[5,5], init = "gauss", width = 0.01, norm='bn', bnl=True):
        ...
        for i in range(c-1):
            layer_list = layer_list + [(('cv'+str(i+1)), BinarizeConv2d(channels[i], channels[i+1], kernel_size=kernels[i], padding=1, bias=False)), 
                                       (('ap'+str(i+1)), torch.nn.AvgPool2d(2,2)), 
                                       ((norm+str(i+1)), self.normalization(channels[i+1], 2, bnl))]
            size = (size +2 - kernels[i])+1
            size = int((size - 2)/2+1)
        
        layer_list = layer_list + [(('fc'+str(c)), BinarizeLinear(size*size*channels[-1], 10, bias=False)),
                                   ((norm+str(c)), self.normalization(10, 1, bnl))]

 
        self.layers = torch.nn.ModuleDict(OrderedDict( layer_list ))

        for key in self.layers.keys():
            if (not(norm in key) and not('ap' in key)):
                if init == "gauss":
    ...
    def normalization(self, size, dim, bnl=True):
        if self.norm=='in':
            if dim==2:
            ...
            if dim==2:
                return torch.nn.BatchNorm2d(size, affine=bnl, track_running_stats=bnl)
            else:
                return torch.nn.BatchNorm1d(size, affine=bnl, track_running_stats=bnl)
    ...
    def forward(self, x):
        ...
        for i in range(c-1):
            ...
            x = self.layers['cv'+str(i+1)](x)
            x = self.layers['ap'+str(i+1)](x)
            x = self.layers[self.norm+str(i+1)](x)
            x = SignActivation.apply(x)

        x = x.view(x.size(0), -1)
        x = self.layers['fc'+str(c)](x)
        if self.norm == 'in':  #IN needs channel dim
            x = self.layers[self.norm+str(c)](x.unsqueeze_(1)).squeeze_(1)
        else:
            x = self.layers[self.norm+str(c)](x)
        return x
