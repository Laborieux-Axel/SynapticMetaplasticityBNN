...
parser.add_argument('--des', type = str, default = '', metavar = 'DS', help='description of the run')
...
parser.add_argument('--ch', nargs = '+', type = int, default = [1,64,64], metavar = 'CH', help='channels for convnet')
parser.add_argument('--kr', nargs = '+', type = int, default = [5,5], metavar = 'KR', help='kernels for convnet')
...
path = 'results/'+date+'/'+time+'_gpu'+str(args.device)+'___'+args.des
...
for ldr_idx in range(len(train_loader_list)):
    ldr = train_loader_list[ldr_idx]
    digits, _ = next(iter(ldr))
    idx = np.random.randint(ldr.batch_size)
    digit = digits[idx].squeeze() #permute(1,2,0)

    fig = plt.figure(figsize=(12,12))
    plt.imshow(digit)
    plt.colorbar()
    fig.savefig(path +'/sample_task'+str(ldr_idx+1)+'.png', fmt='png')



if args.interleaved:
    dset_train = torch.utils.data.ConcatDataset(dset_train_list)
    print('cat dset length  : ', len(dset_train))
    ...
    for dst in dset_train_list:
        print('dset train length : ', len(dst))
...
if args.net =='bnn':
    ...
    model = ConvBNN(channels=args.ch, kernels=args.kr, init = args.init, width = args.init_width, norm=args.norm, bnl=False).to(device)
...
data['arch'] = arch[1:]
data['norm'] = args.norm
data['lr'], data['meta'], data['ewc'], data['SI'], data['task_order'] = [lr], [meta], [ewc_lambda], [si_lambda], [1]  
data['tsk'], data['epoch'], data['acc_tr'], data['loss_tr'] = [args.task_sequence[0]], [0], [10.0], [10.0]


for i in range(len(test_loader_list)):
    data['acc_test_tsk_'+str(i+1)], data['loss_test_tsk_'+str(i+1)] = [10.0], [10.0]
